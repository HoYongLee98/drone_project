...
static void himax_set_timing(struct pi_device *dev, uint16_t frame_len, uint16_t line_len)
{
  uint8_t v;

  // 그룹 홀드(옵션): 한 프레임 경계에서 동시에 반영
  v = 0x01; // hold
  pi_camera_reg_set(dev, 0x0104, &v);

  // FRAME_LENGTH_LINES (0x0340: H, 0x0341: L)
  v = (frame_len >> 8) & 0xFF; pi_camera_reg_set(dev, 0x0340, &v);
  v = (frame_len >> 0) & 0xFF; pi_camera_reg_set(dev, 0x0341, &v);

  // LINE_LENGTH_PCK (0x0342: H, 0x0343: L)
  v = (line_len >> 8) & 0xFF;  pi_camera_reg_set(dev, 0x0342, &v);
  v = (line_len >> 0) & 0xFF;  pi_camera_reg_set(dev, 0x0343, &v);

  v = 0x00; // release
  pi_camera_reg_set(dev, 0x0104, &v);

  // CMU 적용은 N+2 프레임에서 반영됨(데이터시트 8.1). :contentReference[oaicite:2]{index=2}
}
...
static int open_pi_camera_himax(struct pi_device *device)
{
  ...
  himax_set_timing(device, 0x0696, 0x0172);
...
void sendBufferViaCPX_origin(CPXPacket_t * packet, uint8_t * buffer, uint32_t bufferSize) {
  uint32_t offset = 0;
  uint32_t size = 0;
  do {
...
void sendBufferViaCPX(CPXPacket_t *packet, uint8_t *buffer, uint32_t bufferSize) {
  ...
  while (offset < bufferSize) {
    ...
    if (offset + chunk > bufferSize) chunk = bufferSize - offset;
    memcpy(packet->data, &buffer[offset], chunk);   // <<<< 반드시 chunk 사용
...
void setupWiFi_temp(void) {
  static char ssid[] = "RUBIS_001_5G";
  static char pw[] = "159skdlazz@";
  cpxPrintToConsole(LOG_TO_CRTP, "Setting up WiFi AP\n");
  // Set up the routing for the WiFi CTRL packets
  txp.route.destination = CPX_T_ESP32;
  rxp.route.source = CPX_T_GAP8;
  ...
  wifiCtrl->cmd = WIFI_CTRL_SET_SSID;
  memcpy(wifiCtrl->data, ssid, sizeof(ssid));
  txp.dataLength = sizeof(ssid) + 1;
  cpxSendPacketBlocking(&txp);

  wifiCtrl->cmd = WIFI_CTRL_SET_KEY;
  memcpy(wifiCtrl->data, pw, sizeof(pw));
  txp.dataLength = sizeof(pw) + 1;
  cpxSendPacketBlocking(&txp);
...
void setupWiFi(void) {
  static char ssid[] = "RUBIS_001";
  static char pw[] = "159skdlazz@";
  cpxPrintToConsole(LOG_TO_CRTP, "Setting up WiFi STA\n"); // 메시지 변경
  ...
  // SSID 설정
  wifiCtrl->cmd = WIFI_CTRL_SET_SSID;
  memset(wifiCtrl->data, 0, sizeof(wifiCtrl->data));
  memcpy(wifiCtrl->data, ssid, strlen(ssid));
  txp.dataLength = strlen(ssid) + 1;
  cpxSendPacketBlocking(&txp);

  // Password 설정
  wifiCtrl->cmd = WIFI_CTRL_SET_KEY;
  memset(wifiCtrl->data, 0, sizeof(wifiCtrl->data));
  memcpy(wifiCtrl->data, pw, strlen(pw));
  txp.dataLength = strlen(pw) + 1;
  cpxSendPacketBlocking(&txp);
  ...
  while (1)
  {
    if (wifiClientConnected == 1)
    {
      ...
      if (streamerMode == JPEG_ENCODING)
      {
        ...
        // First send header (조각화 전송)
        sendBufferViaCPX(&txp, (uint8_t*)header.data, headerSize);

        // Send image data (조각화 전송)
        sendBufferViaCPX(&txp, (uint8_t*)jpeg_data.data, jpegSize);

        // Send footer (조각화 전송)
        sendBufferViaCPX(&txp, (uint8_t*)footer.data, footerSize);
